...
//					Giving a prompt if fields are empty and a user tries to sign in 
					
					if(user_idField.getText().isEmpty() || passwordField.getText().isEmpty()) {
						JOptionPane myOption = new JOptionPane();
				    	myOption.showMessageDialog(null, "Enter data to Sign In");
					}else {
//						signing the user in if they exist
...
//							signing the user in as an Blood bank if they are one
...
//							signing the user in as a donor or recipient if they are 
							status_id = status_id_to_log_in;
...
//						showing a prompt if the user is fails the sign in process
						JOptionPane myOption = new JOptionPane();
